...
Close prompt attribution in two passes. First, draft all six complete prompts only from explicit facts in the four source streams plus owner-local conservative or creative choices; do not call the draft source-closed yet. Second, freeze the final wording, split every clause in all six prompts into atomic occurrences, and build or expand the inventory from those final atoms: `fact_id | prompt_ref | normalized atom | owner section | source stream | evidence_refs | status`.
